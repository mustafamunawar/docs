...
compiling app.ts:   "tsc app.ts"
auto compiling single file: "tsc app.ts --watch" or "tsc app.ts -w"

for auto-compiling all ts file in a project and other ts compiler configurations:
in project folder: "tsc --init"
it will create "tsconfig.json" configuration file which has a large number of settings with good defaults.
...
by default all tsconfig.json is for  "compilerOptions": {all defaut setting properties} . We can add more settings after/before that.
following are some:

"exclude": [] is an array of files or paths to be excluded (i.e. these .ts files will not be compiled).
...
tsc compile [includes] minus [excludes] so excludes filter includes files/folder
...
.d.ts file is generated by tsc command, it is a TypeScript Declaration file. While compiling a TypeScript file, the compiler can strip all the type information and store it in this declaration file. The generated JavaScript file will not contain any TypeScript specific information.

In general TS implicitly infer a variable type based on initial assigned value e.g.  for "let x = 5" TS assume type is "number".


Or the coder can explicitly assign a type to a variable  via "type annotation" which is like  "variable: type-name" e.g.:
let name: string; or let age: number; etc. Note it is variblefollowed by a colon followed by a space and type-name at the end.

Function parameters type annotation is exactly same as for any other variable.
For function return type annotation place colon+space+type-name(CST) after closing(right) paranthesis. 

function sumTwoNumers(num1: number, num2: number): string {
...
But if noImplicitAny flag is set then it generate an error.
...
type  NewType =  OldYype & {
  newProp: type-name
}

For objects typing it is highly recommended to us interface not type alias.
...
The process of refining types to more specific types than declared via "type guards" (or coditional blocks of code) is called "narrowing". In many editors(of course including vscode) we can observe these types as they change,
