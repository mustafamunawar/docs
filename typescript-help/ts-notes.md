...
The `tsc` command is used to compile .ts files.
...
By default all tsconfig.json is for "compilerOptions": {all default setting properties} . We can add more settings after/before that.
following are some:
...
The .d.ts file is generated by tsc command, it is a TypeScript Declaration file. While compiling a .ts, the compiler can strip all the type information and store it in this declaration file. The generated .js file (JavaScript file) will not contain any TS specific information.
...
let name: string; or let age: number; etc. Note it is variable followed by a colon followed by a space and type-name at the end.
